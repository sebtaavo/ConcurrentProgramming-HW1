...
int main(int argc, char *argv[]) {
    ...
    #pragma omp parallel
    {
        ...
        char resultsBufferBackwardsForSearch[WORD_LENGTH * 2];
        ...
        for(int i = 0; i < wordCount; i++){
            ...
            if(palindromeOrSemordnilaps){ /*this block is entered if the above function did not return NULL (result was found)*/
                ...
                /*we also save the other order string 1 -> string 2 to check if we already saved this palindrome the other way around. new since report.*/
                snprintf(resultsBufferBackwardsForSearch, sizeof(resultsBufferBackwardsForSearch), "%s -> %s", *palindromeOrSemordnilaps, dictionaryInArray[i]);
                int alreadyExists = 0;
                for(int j = 0; j < results_local_count; j++){
                    if(strcmp(results_local[j], resultsBufferBackwardsForSearch)==0){
                        alreadyExists = 1;
                    }
                }
                if(!alreadyExists){
